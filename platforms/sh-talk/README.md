...
## 기능
- 휴대폰형 화면, 하단 탭: 사무실(첫 화면) / 채팅 / 공지
- 사무실: 유리벽 복도에 팀장실 3개(전략기획팀장실 머스크, 감사팀장실 버핏, 기술팀장실 잡스)와 대회의실. 방을 누르면 유리문이 열리고 그 팀장과의 1:1 방으로 들어감(뒤로가기 = 복도)
- 대회의실(= 전체방): "팀장 소집"을 누르면 세 팀장이 차례로 들어와 앉고, 이후 메시지에는 머스크 → 버핏 → 잡스 순서로 답함(회의 모드, 이 브라우저에만 기억). 입장 안내 줄은 화면 표시만 하고 DB에 저장하지 않음
