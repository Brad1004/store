...
| SalamSeleb 데스크탑 홈페이지 | 2호 SSN 홈페이지 사내 비공개 시안 (히어로·축하 상황·샘플 Seleb 17명 분야별 보기·신청→수락→전달→정산 4단계·안심 결제·Seleb 되기·등급·기업 고객·FAQ, 한국어/인니어/영어, 휴대폰 화면 대응, 개인정보 수집 없음) | `platforms/salamseleb-web/` (빌드: `src/build.py`) | v0.1 게시 (비공개, 공개 전 상표 출원·법인 설립 필요) | https://claude.ai/artifact/GREP4cSG8B2KGhkA6qcjr7 |
| SalamSeleb 모바일 홈페이지 | 2호 SSN 모바일 홈페이지 사내 비공개 시안 (데스크탑과 같은 문구·데이터, 옆으로 넘기는 Seleb 카드·분야 버튼, 세로 4단계, 메뉴·언어 선택 창, 하단 고정 버튼 'Seleb 둘러보기/Seleb 되기', 한국어/인니어/영어, 개인정보 수집 없음) | `platforms/salamseleb-web/mobile.html` (빌드: `src/build.py`, 공용 문구 `src/copy.js`) | v0.1 게시 (비공개) | https://claude.ai/artifact/1msrmq1fjRVFni8bQibQwj |
| Tokokita 앱 MVP | 3호 가게(A)·손님(B) 등록·로그인 전환, 가게 대표 음식 사진(자동 배정), 첫 화면 "동네 가게 살리는 할인 쿠폰" 소개(사진·할인 배지), 시간대별 할인표, 예약 코드, 사장님 코드 확인·노쇼, 추천 시간표, 가게 사진첩(대표+메뉴·내부 최대 6장, 더보기), Google 지도 보기·길찾기, 별점, 한/인니/영 | `platforms/tokokita-app/` | v0.2 게시 (비공개) | https://claude.ai/artifact/F7uLjuAQiiX9bLKfKdUge3 |
